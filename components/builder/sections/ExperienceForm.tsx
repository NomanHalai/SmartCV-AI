  ...
  return (
    <div className="mt-2 p-3 bg-blue-50 border border-blue-100 rounded-lg">
      ...
      <div className="flex gap-2">
        <input
          className="input-base text-xs flex-1 bg-white"
      ...
      {suggestions.length > 0 && (
        <div className="mt-2 space-y-1.5">
          {suggestions.map((s, i) => (
            <div key={i} className="flex items-start gap-2 p-2 bg-white rounded border border-blue-100 hover:border-blue-300 transition-colors">
  ...
  return (
    <div className="relative">
      ...
      {show && suggestions.length > 0 && (
        <div className="absolute left-0 top-6 z-20 w-80 bg-white border border-purple-200 rounded-xl shadow-xl p-3">
          <div className="flex justify-between items-center mb-2">
            <p className="text-xs font-semibold text-purple-700">Improved versions</p>
            <button onClick={() => setShow(false)} className="text-slate-400 hover:text-slate-600 text-xs">✕</button>
          </div>
          {suggestions.map((s, i) => (
            <div key={i} className="p-2 text-xs text-slate-700 border border-slate-100 rounded-lg mb-1.5 hover:border-purple-300 cursor-pointer transition-colors leading-relaxed"
      ...
      {open && (
        <div className="px-4 pb-4 space-y-3 border-t border-slate-100 pt-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <label htmlFor={`current-${exp.id}`} className="text-xs text-slate-500">I currently work here</label>
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="text-xs font-medium text-slate-500 uppercase tracking-wider">
                Key Achievements
                <span className="ml-1 text-slate-400 normal-case tracking-normal font-normal">(use numbers + action verbs)</span>
              </label>
              <button
                onClick={() => setShowBulletAI(!showBulletAI)}
                className={`text-xs flex items-center gap-1 font-medium transition-colors ${showBulletAI ? 'text-blue-600' : 'text-slate-400 hover:text-blue-500'}`}
              >
                <Sparkles size={11} />
                AI Generate
              </button>
...
export function ExperienceForm() {
  ...
  return (
    <div>
      ...
      <button
        onClick={handleAdd}
        className="w-full py-3 rounded-xl border-2 border-dashed border-slate-200 text-sm text-slate-400 hover:border-teal-300 hover:text-teal-600 hover:bg-teal-50 transition-all flex items-center justify-center gap-2 mb-4"
      >
